...
import type { RegistryCache, CacheStore } from "./cache.js";
import type { RegistryEntry, SyncState } from "./events.js";

export class LocalRegistryCache implements RegistryCache {
  ...
  exportCache(): CacheStore { return JSON.parse(JSON.stringify({ entries: this.cache.entries, updatedAt: this.cache.updatedAt })); }
  importCache(data: unknown): void {
    const v = validateCache(data);
    if (!v.valid) throw new Error(`Invalid cache data: ${v.errors.join(", ")}`);
    this.cache = (data as CacheStore); this.save();
  }

  clear(): void { this.cache = { entries: {}, updatedAt: 0 }; try { const p = this.path("cache.json"); if (existsSync(p)) unlinkSync(p); } catch { /* */ } }

  loadSyncState(sourceId: string): SyncState | null {
    const p = this.path("sync-state.json");
    if (!existsSync(p)) return null;
    try { const data = JSON.parse(readFileSync(p, "utf-8")); return data[sourceId] ?? null; }
    catch { return null; }
  }
